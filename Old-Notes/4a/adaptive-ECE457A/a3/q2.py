...
def aco(num_ants=len(cities), online_update=True, alpha=1.0, beta=1.0, rho=0.5,
        iterations=100):
    Ant.ALPHA = alpha
    Ant.BETA = beta
    Ant.RHO = rho
    ...
    title = 'n{}-a{}-b{}-r{}{}.png'.format(
        num_ants, Ant.ALPHA, Ant.BETA, Ant.RHO, '-online' if online_update else '')
    print num_ants, " & ", Ant.ALPHA, " & ", Ant.BETA, " & ", Ant.RHO, " & ", online_update, " & ", '{0:.2f}'.format(current)
    ...
    for alpha in (0.2, 0.5, 1):
        for beta in (0.2, 0.5, 1):
            for rho in (0.4, 0.5, 0.6):
                for num_ants in (len(cities)/2, len(cities), len(cities)*2):
                    for online_update in (False, True):
                        aco(num_ants=num_ants, online_update=online_update, alpha=alpha,
                            beta=beta, rho=rho)
